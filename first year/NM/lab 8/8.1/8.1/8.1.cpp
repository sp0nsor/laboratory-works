	...
	void check_diagonal_dominance() {
		for (int i = 0; i < this->N; i++) {
			...
			if (fabs(this->table[i][i]) <= fabs(sum)) {
				...
				throw invalid_argument("Не удовлетворяет условию скодимости");
			}
		}
		cout << "Удовлетворяет условию скодимости" << endl;
	}
...
public:
	Matrix(int n, int m) {
		if (n <= 0 || m <= 0) {
			throw invalid_argument("N и M должно быть строго больше 0");
		}
	...
	void resolve(long double exactness) {
		...
		cout << "\nУравнение решено точностью  [" << exactness << "], номер итерации {" << counter - 1 << "}:" << endl;
		for (int i = 0; i < this->M - 1; i++) {
			this->X[i] = Xi[i];
